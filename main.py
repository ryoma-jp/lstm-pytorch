...
from lstm import evaluate as lstm_evaluate
...
def parse_args():
    ...
    """
    parser = argparse.ArgumentParser(description="Impremenation of LSTM for Time Series Analysis")
...
def main():
    ...
    # --- Training LSTM ---
    model = lstm_train(X_train, y_train, save_path=args.output_dir)
    train_pred = lstm_predict(model, X_train)
    train_mse = lstm_evaluate(y_train, train_pred)
    print(f'Train MSE: {train_mse}')

    # --- Test LSTM ---
    test_pred = lstm_predict(model, X_test)
    test_mse = lstm_evaluate(y_test, test_pred)
